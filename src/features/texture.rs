...
impl FeatureSet for GlcmFeatureSet {
    fn compute_features_batched(
        ...
        for (offset, glcm) in glcms.iter() {
            ...
            let levels =
                Tensor::arange(GLCM_LEVELS as i64, (Kind::Float, patchs.device())).unsqueeze(0); // [1, LEVEL]
            ...
            let (pxpy, pxdy) = {
                ...
                let pxpy = Tensor::zeros(
                    &[batch_size, GLCM_LEVELS as i64 * 2 - 2],
                    (Kind::Float, patchs.device()),
                ); // [N, LEVEL * 2 - 2]
                let pxdy = Tensor::zeros(
                    &[batch_size, GLCM_LEVELS as i64],
                    (Kind::Float, patchs.device()),
                ); // [N, LEVEL]
            ...
            let correlation =
                {
                    //[N]
                    let intensity =
                        Tensor::arange(GLCM_LEVELS as i64, (Kind::Float, patchs.device()));
            ...
            let (contrast, dissimilarity) =
                {
                    let i = Tensor::arange(GLCM_LEVELS as i64, (Kind::Float, patchs.device()))
                        .view([1, 1, GLCM_LEVELS as i64]);
                    let j = Tensor::arange(GLCM_LEVELS as i64, (Kind::Float, patchs.device()))
                        .view([1, GLCM_LEVELS as i64, 1]);
            ...
            let (sum_average, sum_variance) = {
                // [N], [N]
                let k =
                    (Tensor::arange(GLCM_LEVELS as i64 * 2 - 2, (Kind::Float, patchs.device()))
                        + 2)
                    .unsqueeze(0);
            ...
            let (difference_average, difference_variance) = {
                let k =
                    Tensor::arange(GLCM_LEVELS as i64, (Kind::Float, patchs.device())).unsqueeze(0);
...
impl GlcmFeatures {
    pub fn write_header_to_csv<W: std::io::Write>(
        ...
        let headers = OFFSETS
            ...
            .flat_map(|offset| {
                let (dx, dy) = offset;
                GlcmFeatures::FIELD_NAMES_AS_ARRAY
                    .iter()
...
pub fn glcm_features(
    patches: &Tensor,
    masks: &Tensor,
) -> (Vec<Vec<GlcmFeatures>>, Vec<(i64, i64)>) {
    let _ = tch::no_grad_guard();
    ...
    for glcm in glcms.iter() {
        ...
        let levels =
            Tensor::arange(GLCM_LEVELS as i64, (Kind::Float, patches.device())).unsqueeze(0); // [1, LEVEL]
        let intensity_x = (&levels * &px).sum_dim_intlist(Some(&[-1][..]), false, Kind::Float); // [N]
        let intensity_y = (&levels * &py).sum_dim_intlist(Some(&[-1][..]), false, Kind::Float); // [N]
        ...
        let correlation = {
            ...
            let intensity = intensity
                ...
                .matmul(&intensity.unsqueeze(0))
                .unsqueeze(0); // [1, LEVEL, LEVEL]
            (glcm * intensity - (&intensity_x * &intensity_y).view([-1, 1, 1])).sum_dim_intlist(
                Some(&[-1, -2][..]),
                false,
                Kind::Float,
            )
        ...
        let (contrast, dissimilarity) = {
            let i = Tensor::arange(GLCM_LEVELS as i64, (Kind::Float, patches.device())).view([
                1,
                1,
                GLCM_LEVELS as i64,
            ]);
            let j = Tensor::arange(GLCM_LEVELS as i64, (Kind::Float, patches.device())).view([
                1,
                GLCM_LEVELS as i64,
                1,
            ]);
        ...
        let entropy =
            -(glcm * (glcm + 1e-6).log2()).sum_dim_intlist(Some(&[-1, -2][..]), false, Kind::Float);
        ...
        let (sum_average, sum_variance) = {
            // [N], [N]
            let k = (Tensor::arange(GLCM_LEVELS as i64 * 2 - 2, (Kind::Float, patches.device()))
                + 2)
            .unsqueeze(0);
            let sum_avg = (&k * &pxpy).sum_dim_intlist(Some(&[-1][..]), false, Kind::Float); // [N]
            let sum_var = ((k - sum_avg.unsqueeze(1)).square() * &pxpy).sum_dim_intlist(
        ...
        let sum_entropy =
            -(&pxpy * (&pxpy + 1e-6).log2()).sum_dim_intlist(Some(&[-1][..]), false, Kind::Float);

        let sum_of_squares = {
            let i = Tensor::arange(GLCM_LEVELS as i64, (Kind::Float, patches.device()))
                .view([1, GLCM_LEVELS as i64]);
            let var = (i - intensity_x.unsqueeze(1)).square().unsqueeze(-1) * glcm;
            var.sum_dim_intlist(Some(&[-1, -2][..]), false, Kind::Float)
        };
        ...
        let inverse_difference_moment = {
            let i = Tensor::arange(GLCM_LEVELS as i64, (Kind::Float, patches.device()))
                .view([1, -1, 1]);
            let j = Tensor::arange(GLCM_LEVELS as i64, (Kind::Float, patches.device()))
                .view([1, 1, -1]);
            let imj = (i - j).square();
            let idm = glcm / (imj + 1.0);
            idm.sum_dim_intlist(Some(&[-1, -2][..]), false, Kind::Float)
        ...
        let (difference_average, difference_variance) = {
            let k =
                Tensor::arange(GLCM_LEVELS as i64, (Kind::Float, patches.device())).unsqueeze(0);
            let da = (&k * &pxdy).sum_dim_intlist(Some(&[-1][..]), false, Kind::Float); // [N]
            let dva = ((k - da.unsqueeze(-1)).square() * &pxdy).sum_dim_intlist(
                Some(&[-1][..]),
                false,
...
impl GLRLMFeatures {
    pub fn write_header_to_csv<W: std::io::Write>(
        ...
        let headers = OFFSETS
            ...
            .flat_map(|offset| {
                let (dx, dy) = offset;
                GLRLMFeatures::FIELD_NAMES_AS_ARRAY
                    .iter()
                    .map(move |field_name| format!("{field_name}_{dx}_{dy}"))
            })
            .collect::<Vec<_>>();
...
pub fn glrlm_features(patches: &Tensor, masks: &Tensor) -> Vec<Vec<GLRLMFeatures>> {
    ...
    for direction in &DIRECTIONS {
        ...
        // Short Run Emphasis [N] & Long Run Emphasis [N]
        let (short_run_emphasis, long_run_emphasis) = {
            let j = Tensor::arange(GLRLM_MAX_LENGTH, (Kind::Float, patches.device())).unsqueeze(0)
                + 1.0;
            let j2 = j.square();
            let sre = &rlrnv / &j2;
            let lre = &rlrnv * j2;
        ...
        for i in 0..batch_size {
            ...
            let short_run_extreme_gray_level_emphasis =
                f32::from(short_run_extreme_gray_level_emphasis.i(i));
            let long_run_extreme_gray_level_emphasis =
                f32::from(long_run_extreme_gray_level_emphasis.i(i));
            ...
            glrlms[i as usize].push(GLRLMFeatures {
                short_run_emphasis,
                long_run_emphasis,
    ...
    pub fn get_headers() -> Vec<String> {
        ...
        for angle in 0..ANGLES_COUNT {
            for frequency in 0..FREQUENCIES.len() {
                headers.push(format!(
                    "gabor_angle_{}_frequency_{}_mean",
                    angle, FREQUENCIES[frequency]
                ));
                headers.push(format!(
                    "gabor_angle_{}_frequency_{}_variance",
                    angle, FREQUENCIES[frequency]
                ));
            }
        }
