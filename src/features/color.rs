...
pub fn mean_std(img: &Tensor, mask: &Tensor) ->  ((f32, f32, f32), (f32, f32, f32)){
    ...
    let mask_area /* [3] */ = mask.sum_dim_intlist(Some(&[-1i64, -2][..]), false, tch::Kind::Float);
    let mut mean /* [3] */ = masked.sum_dim_intlist( Some(&[-1i64, -2][..]), false, tch::Kind::Float);
    mean /= &mask_area;
    let std /* [3, H, W] */ = img - &mean;
    let std = &std * &std * mask;
    let mut std = std.sum_dim_intlist(Some(&[-1i64, -2][..]), false, tch::Kind::Float);
    std /= &mask_area;
